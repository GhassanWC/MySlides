...
If the user wants to add an image/icon to the slide and it exists in Available Uploaded Assets, you MUST natively inject the "url" of the matched asset directly into your generated "html" (e.g. using <img src="...">) or "css" (e.g. background-image) so that the layout beautifully and natively incorporates their specific uploaded asset.
DO NOT output an "elements" array. The assets must be integrated cleanly within the DOM structure to achieve premium visual harmony.
...
- If asked to add a SPECIFIC asset that you see in "Available Uploaded Assets" (like "add my logo" or "insert mountain image"), you MUST construct a beautiful wrapping <div> inside the HTML and embed the asset "url" natively. Redesign the HTML layout to comfortably hold the new object using flexbox/grid!
- Do NOT hallucinate images unless you are using reliable Unsplash placeholders or URLs directly from the Available Assets.
